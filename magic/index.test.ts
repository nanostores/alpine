...
it('returns current atom value', async () => {
  ...
  expect(getEl(body, '#out').textContent).toBe('7')
})
...
it('per-element cache: same store on same element creates one binding', async () => {
  ...
  let el = getEl<{ _nanostores: Map<unknown, unknown> } & Element>(body, '#el')
  expect(el._nanostores.size).toBe(1)
})
...
it('supports x-show with $nano', async () => {
  ...
  expect(getEl<HTMLElement>(body, '#el').style.display).not.toBe('none')

  $visible.set(false)
  await Alpine.nextTick()

  expect(getEl<HTMLElement>(body, '#el').style.display).toBe('none')
})
...
it('supports attribute binding with $nano', async () => {
  ...
  expect(getEl<HTMLButtonElement>(body, '#btn').disabled).toBe(false)

  $disabled.set(true)
  await Alpine.nextTick()

  expect(getEl<HTMLButtonElement>(body, '#btn').disabled).toBe(true)
})
